...
# ==============================
# ATUALIZAÇÃO E AUTO-REFRESH
# ==============================
if st.sidebar.button("🔄 Atualizar Agora"):
    st.cache_data.clear()
    st.rerun()

# Auto-refresh de 1 minuto para esta tela
...
def buscar_dados_cana(data_filtro):
    ...
    try:
        resposta = (
            supabase
            .table("APP COLHEITA")
            .select("frente, nome_fazenda, gleba, atr, mineral_pct, vegetal_pct, tc_real, tc_estimado")
            .eq("data_saida", data_str)
            .execute()
        )
        return resposta.data
    except Exception as erro:
        st.error(f"Erro consulta dia: {erro}")
        return []
...
def buscar_historico_glebas_ativas(lista_glebas):
    if not lista_glebas:
        return pd.DataFrame()
    try:
        lista = [int(x) for x in lista_glebas if pd.notna(x)]
        resposta = (
            supabase
            .table("APP COLHEITA")
            .select("gleba, tc_real")
            .in_("gleba", lista)
            .execute()
        )
        if resposta.data:
            ...
            df["tc_real"] = pd.to_numeric(df["tc_real"], errors="coerce").fillna(0)
            
            acumulado = df.groupby("gleba")["tc_real"].sum().reset_index()
            acumulado.columns = ["gleba", "TC Total Gleba (Histórico)"]
            return acumulado
        return pd.DataFrame()
    except Exception as erro:
        st.error(f"Erro histórico: {erro}")
        return pd.DataFrame()
...
if not dados_banco:
    ...
    df_dia[colunas_num] = df_dia[colunas_num].apply(pd.to_numeric, errors="coerce").fillna(0)

    # Histórico das Glebas
    glebas_do_dia = df_dia["gleba"].dropna().unique().tolist()
    df_historico = buscar_historico_glebas_ativas(glebas_do_dia)

    # Filtro Dinâmico de Frentes na Sidebar
    ...
    if not df_historico.empty:
        df_visualizacao = pd.merge(df_filtrado, df_historico, on="gleba", how="left")
    else:
        df_visualizacao = df_filtrado.copy()
        df_visualizacao["TC Total Gleba (Histórico)"] = 0

    df_visualizacao["TC Total Gleba (Histórico)"] = df_visualizacao["TC Total Gleba (Histórico)"].fillna(0)
    ...
    ordem = ["Frente", "Fazenda", "Gleba", "TC Estimado", "TC (Dia)", "TC (Acumulado)", "ATR", "Imp. Mineral", "Imp. Vegetal"]
    df_visualizacao = df_visualizacao[ordem].sort_values(by=["Frente", "Fazenda", "Gleba"])
    df_visualizacao["Gleba"] = df_visualizacao["Gleba"].fillna(0).astype(int).astype(str)

    # ==============================
    # EXIBIÇÃO: METRICAS E ABAS
    # ==============================
    col1, col2, col3, col4 = st.columns(4)
    col1.metric("🚜 TC Hoje", f"{df_visualizacao['TC (Dia)'].sum():,.2f}")
    col2.metric("🚜 Frentes", df_visualizacao["Frente"].nunique())
    col3.metric("🏭 Fazendas", df_visualizacao["Fazenda"].nunique())
    
    media_atr = df_visualizacao[df_visualizacao["ATR"] > 0]["ATR"].mean()
    col4.metric("📈 Média ATR", f"{media_atr:.2f}" if pd.notna(media_atr) else "0.00")
    ...
    with aba_grafico:
        df_grafico = df_visualizacao.groupby("Frente")["TC (Dia)"].sum().reset_index()
        st.bar_chart(df_grafico, x="Frente", y="TC (Dia)")
